...
func (p builtinTranscriptParser) Parse(file snapshot.TranscriptFile) (*snapshot.SessionTrace, error) {
	if p.parse == nil {
...
func (p builtinTranscriptParser) ParseTail(file snapshot.TranscriptFile) (*snapshot.SessionTrace, error) {
	if p.parseTail == nil {
...
func (p builtinTranscriptParser) ParseAppend(file snapshot.TranscriptFile, base *snapshot.SessionTrace, offset int64) (*snapshot.SessionTrace, error) {
	if p.append == nil {
...
func (p builtinTranscriptParser) CanAppend(file snapshot.TranscriptFile) bool {
...
func newCodexTranscriptParser() agentTranscriptParser {
	return builtinTranscriptParser{
		...
		parseTail: func(file snapshot.TranscriptFile) (*snapshot.SessionTrace, error) {
			if isCodexLaneTranscript(file.Path) {
				return parseCodexLaneTrace(file.Path)
			}
		...
		canAppend: func(file snapshot.TranscriptFile) bool { return !isCodexLaneTranscript(file.Path) },
	}
}
